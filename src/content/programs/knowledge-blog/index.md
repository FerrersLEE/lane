...
**经验教训**：内容驱动的站点，所有列表类的输出（RSS、sitemap、搜索索引）都应该从内容集合动态生成，绝不能手写。
...
**经验教训**：静态站点的搜索不能用后端数据库，常见方案有三种：构建时生成索引前端搜（我们用的这种，适合小到中型站点）、Algolia 第三方服务、Pagefind 等专门工具。
